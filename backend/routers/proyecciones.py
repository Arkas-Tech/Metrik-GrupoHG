...
class ProyeccionRequest(BaseModel):
    ...
    excede_presupuesto: Optional[bool] = False
...
async def create_proyeccion(user: user_dependency, db: db_dependency, proyeccion_request: ProyeccionRequest):
    ...
    proyeccion_model = Proyecciones(
        ...
        estado=proyeccion_request.estado if proyeccion_request.estado else 'pendiente',
        partidas_json=proyeccion_request.partidas_json,
        excede_presupuesto=proyeccion_request.excede_presupuesto,
...
async def update_proyeccion(user: user_dependency, db: db_dependency, 
    ...
    proyeccion.excede_presupuesto = proyeccion_request.excede_presupuesto
...
@router.post("/{proyeccion_id}/aprobar", status_code=status.HTTP_200_OK)
async def aprobar_proyeccion(user: user_dependency, db: db_dependency, proyeccion_id: int = Path(gt=0)):
    """Endpoint para que gerentes/administradores aprueben proyecciones"""
    if user is None:
        raise HTTPException(status_code=401, detail='Authentication Failed')
    ...
    if proyeccion.estado == 'aprobada':
        raise HTTPException(status_code=400, detail='Esta proyección ya está aprobada')
    
    # Aprobar la proyección
    proyeccion.estado = 'aprobada'
    proyeccion.autorizada_por = user.get('username')
    proyeccion.fecha_autorizacion = datetime.now()
    ...
    return {
        "message": "Proyección aprobada exitosamente",
        "proyeccion_id": proyeccion.id,
        "estado": proyeccion.estado,
        "autorizada_por": proyeccion.autorizada_por
    }